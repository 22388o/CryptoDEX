...
VERSION=
commit=false
url=https://github.com/cryptodexproject/cryptodex
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the cryptodex, gitian-builder, gitian.sigs, and cryptodex-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/cryptodexproject/cryptodex
...
then
	# Make output folder
	mkdir -p "./cryptodex-binaries/${VERSION}"
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit cryptodex=${COMMIT} --url cryptodex=${url} ../cryptodex/contrib/gitian-descriptors/gitian-linux.yml
	    ...
	    mv build/out/cryptodex-*.tar.gz build/out/src/cryptodex-*.tar.gz ../cryptodex-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit cryptodex=${COMMIT} --url cryptodex=${url} ../cryptodex/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/cryptodex-*.zip build/out/cryptodex-*.exe ../cryptodex-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit cryptodex=${COMMIT} --url cryptodex=${url} ../cryptodex/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/cryptodex-*.tar.gz build/out/cryptodex-*.dmg ../cryptodex-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit cryptodex=${COMMIT} --url cryptodex=${url} ../cryptodex/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../cryptodex/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/cryptodex-*.tar.gz build/out/src/cryptodex-*.tar.gz ../cryptodex-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../cryptodex/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/cryptodex-*win64-setup.exe ../cryptodex-binaries/${VERSION}
	    mv build/out/cryptodex-*win32-setup.exe ../cryptodex-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../cryptodex/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/cryptodex-osx-signed.dmg ../cryptodex-binaries/${VERSION}/cryptodex-${VERSION}-osx.dmg
	fi
	popd || exit
