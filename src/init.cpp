...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, http, leveldb, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                             "cryptodex (or specifically: gobject, instantsend, keepass, masternode, mnpayments, mnsync, privatesend, spork)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all CryptoDex specific functionality (Masternodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N CXD anonymized (default: %u)"), DEFAULT_PRIVATESEND_AMOUNT));
...
std::string LicenseInfo()
{
    ...
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2014-%i The CryptoDex Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that CryptoDex Core is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("CryptoDex Core version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
}

/** Initialize CryptoDex Core.
...
#define PROCESS_DEP_ENABLE 0x00000001
...
#endif
    // Make sure only a single CryptoDex Core process is using the data directory.
    ...
    try {
        ...
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. CryptoDex Core is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    // ********************************************************* Step 11c: update block tip in CryptoDex modules
    ...
    // ********************************************************* Step 11d: start cryptodex-ps-<smth> threads
