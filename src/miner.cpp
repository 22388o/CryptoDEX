        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    // zCDX spends can have very large priority, use non-overflowing safe functions
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("CryptoDEXMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("CryptoDEXMiner : ProcessNewBlock, block not accepted");
...
void BitcoinMiner(CWallet* pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running CryptoDEXMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
