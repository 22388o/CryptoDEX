...
const char * const BITCOIN_CONF_FILENAME = "cryptodex.conf";
const char * const BITCOIN_PID_FILENAME = "cryptodexd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "cryptodex" is a composite category enabling all CryptoDex-related debug output
            if(ptrCategory->count(string("cryptodex"))) {
...
#else
    const char* pszModule = "cryptodex";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CryptoDexCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CryptoDexCore
    // Mac: ~/Library/Application Support/CryptoDexCore
    // Unix: ~/.cryptodexcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CryptoDexCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CryptoDexCore";
#else
    // Unix
    return pathRet / ".cryptodexcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty cryptodex.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override cryptodex.conf
