...
static CSemacryptodex* semOutbound = NULL;
...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (CryptoDEX users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "CryptoDEX " + FormatFullVersion();
...
void static ProcessOneShot()
{
    ...
    CAddress addr;
    CSemacryptodexGrant grant(*semOutbound, true);
    if (grant) {
...
void ThreadOpenConnections()
{
    ...
    while (true) {
        ...
        CSemacryptodexGrant grant(*semOutbound);
        boost::this_thread::interruption_point();
...
bool OpenNetworkConnection(const CAddress& addrConnect, CSemacryptodexGrant* grantOutbound, const char* pszDest, bool fOneShot)
{
    //
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. CryptoDEX Core is probably already running."), addrBind.ToString());
        else
...
void StartNode(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    if (semOutbound == NULL) {
        // initialize semacryptodex
        int nMaxOutbound = min(MAX_OUTBOUND_CONNECTIONS, nMaxConnections);
        semOutbound = new CSemacryptodex(nMaxOutbound);
    }
