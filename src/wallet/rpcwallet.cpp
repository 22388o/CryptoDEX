...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nDEPRECATED. Returns the current CryptoDex address for receiving payments to this account.\n"
            "\nArguments:\n"
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 7)
        throw runtime_error(
            "sendtoaddress \"cryptodexaddress\" amount ( \"comment\" \"comment-to\" subtractfeefromamount use_is use_ps )\n"
            ...
            "                             The recipient will receive less amount of CryptoDex than you enter in the amount field.\n"
...
UniValue instantsendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "instantsendtoaddress \"cryptodexaddress\" amount ( \"comment\" \"comment-to\" subtractfeefromamount )\n"
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "getreceivedbyaddress \"cryptodexaddress\" ( minconf addlockconf )\n"
            "\nReturns the total amount received by the given cryptodexaddress in transactions with specified minimum number of confirmations.\n"
            "\nArguments:\n"
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"tocryptodexaddress\" amount ( minconf addlockconf \"comment\" \"comment-to\" )\n"
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a cryptodex address."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 8)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount     (numeric or string) The cryptodex address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value\n"
            "      ,...\n"
            "    }\n"
            ...
            "                           Those recipients will receive less cryptodexs than you enter in their corresponding amount field.\n"
...
UniValue walletpassphrase(const UniValue& params, bool fHelp)
{
    ...
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending cryptodexs\n"
            "\nArguments:\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    ...
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending cryptodex\n"
    ...
    StartShutdown();
    return "Wallet encrypted; CryptoDex Core server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
...
UniValue lockunspent(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending cryptodexs.\n"
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "3. \"addresses\"      (string) A json array of cryptodex addresses to filter\n"
            "    [\n"
            "      \"address\"     (string) cryptodex address\n"
            "      ,...\n"
            "    ]\n"
            "\nResult\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the cryptodex address\n"
    ...
    if (params.size() > 2) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid CryptoDex address: ")+input.get_str());
            if (setAddress.count(address))
