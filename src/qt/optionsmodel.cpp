...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::CDX);
    ...
    if (!settings.contains("nAnonymizeCryptoDEXAmount"))
        settings.setValue("nAnonymizeCryptoDEXAmount", 1000);

    nAnonymizeCryptoDEXAmount = settings.value("nAnonymizeCryptoDEXAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeCryptoDEXAmount"))
        SoftSetArg("-anonymizecryptodexamount", settings.value("nAnonymizeCryptoDEXAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in cryptodex.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCryptoDEXAmount:
            return QVariant(nAnonymizeCryptoDEXAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCryptoDEXAmount:
            nAnonymizeCryptoDEXAmount = value.toInt();
            settings.setValue("nAnonymizeCryptoDEXAmount", nAnonymizeCryptoDEXAmount);
            emit anonymizeCryptoDEXAmountChanged(nAnonymizeCryptoDEXAmount);
            break;
        case CoinControlFeatures:
